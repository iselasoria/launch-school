=begin 
https://edabit.com/challenge/2WcypxLy2SCFYZWXu
...
Rules:
- strangers until acquaintances, and acquaintance until friend 
- strangers:
  - words encountered for the first time ONCE
- acquaintance:
  - words encountered THREE times 
- friends:
  - words encountered FIVE times 

- resulting nested array 
  - first subarray is to be an array of acquaintances in the order they became acquaintances
  - second subarray is to be an array of friends in the order they became friends 
- as a word goes from stranger, to acquaintance, to friend, it gets deleted from the previous category--> a word can only be one thing 


Examples:
'See Spot run. See Spot jump. Spot likes jumping. See Spot fly.'
 1.   1     1    2  2.   1.    3.   1.    1.       3.   4.  1
...
#[[acquaintances][friends]] --> [['spot', 'see'],[]]
** MUST RETAIN ORDER **


DS: 
hash to count occurences of a word 


Algorithm:
- delete all but alpha characters and "'"
- split str into `words_arr`

- init an empty hash object with default values as 0
- iterate over `words_arr`
  - with each iteration 
    - set word as key and count as value 
    - check 
      - if the current count in the hash is greater than or equal to 5, store in `acq_arr`
      - if the current count is five or more, store in `friends_arr`

return [acq_arr, friends_arr]
...
def no_strangers(str)
  ...
  words_arr.each do |word|
    hash_counts[word] += 1
    if hash_counts[word] >= 3 && hash_counts[word] < 5
      acq_arr << word
    elsif hash_counts[word] >= 5
      friends_arr << word 
    end
  end
  p hash_counts
  # remove from acq_arr 
  # select from acq_arr all those not friends 
  acq_arr.select! {|item| !friends_arr.include?(item)}

  [acq_arr.uniq!, friends_arr]
end
...
# p no_strangers('See Spot run. See Spot jump. Spot likes jumping. See Spot fly.') == [['spot', 'see'],[]]
p no_strangers(t1) #== [['says', 'a', 'good', 'boy', "doesn't", 'ask', 'again', 'is', 'child'], ['his', 'mother', 'to', 'billy', 'she']]
# p no_strangers(t2) == [['be', 'and', 'need', 'to', 'it', 'the', 'like', 'a', 'for', 'with'], ['you', "don't"]]
# p no_strangers(t3) == [['the', 'ethical', 'nonmonogamy', 'making', 'family', 'very', 'important', 'down', 'earth', 'and', 'not', 'penpal', 'of', 'if', 'you', 'something'], ['a', 'to', 'i', 'for', 'looking', 'as', 'my', 'is', "i'm", 'me']]
# p no_strangers(t4) == [['sophisticated', 'delightful', 'carefully', 'uniforms', 'flat', 'white', 'exclusive', 'discerning', 'sharp', 'business', 'class', 'boutique', 'perfect', 'ryokan'], ['extraordinary', 'curated', 'destination']]
# p no_strangers(t5) == [['i', 'of', 'coffee', 'me', 'home', 'if', 'kurosawa'], ['a', 'my']]