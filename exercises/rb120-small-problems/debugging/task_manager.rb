...
class TaskManager
  ...
  def display_high_priority_tasks
    high_priority_tasks = tasks.select do |task| # this used to read tasks = tasks.select
      task.priority == :high
    end

    display(high_priority_tasks) # this used to take tasks as the argument
  end
...
# S:
# the solution is to disambiguate between the tasks on the left hand side of the equal sign, and
# that on the right hand side. Essentially, having the code read:
def display_high_priority_tasks
  tasks = tasks.select do |task|
    task.priority == :high
  end
end
# means that the `tasks` getter method on the right hand side of the equal sign is shadowed by the
# local variable `tasks` on the left hand side, and thus Ruby freaks out and just treats them both as
# local variables